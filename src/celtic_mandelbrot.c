...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/24 21:03:17 by hkuhic            #+#    #+#             */
/*   Updated: 2019/09/27 21:17:45 by hkuhic           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calculate_coor_for_myfractol(t_fractol *fr)
{
	...
	{
		fr->z = complex(fabs(fr->z.re * fr->z.re - fr->z.im * fr->z.im) +
		fr->c.re, 2 * fr->z.re * fr->z.im + fr->c.im);
		fr->interation++;
	}
...
void	myfractol(t_fractol *fr)
{
	while (fr->y < HEIHGT)
	{
		...
		while (fr->x < WIDTH)
		{
			...
			fr->y / fr->zoom + fr->y1);
			fr->z = complex(0, 0);
			calculate_coor_for_myfractol(fr);
			fr->x++;
		}
		fr->y++;
	}
